...
export class BConnectClient {
  ...
  public operatingSystems: OperatingSystemsModule;
  ...
  constructor(config: BConnectConfig) {
    ...
    this.operatingSystems = new OperatingSystemsModule(this.client);
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      await this.operatingSystems.getFolders({ PageSize: 1 });
      return true;
    } catch (error) {
