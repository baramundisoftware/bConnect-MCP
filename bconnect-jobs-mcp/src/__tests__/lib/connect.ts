 ...
 *
 * Upstream finding OPT-39 (B1). createServer() used to override
 * `server.request()` and reach into the SDK's private `_requestHandlers` map so
 * tests could dispatch handlers directly. That override shipped in production
 * and depended on an undocumented SDK internal. It is gone; this helper
 * replaces it with the InMemoryTransport pair the SDK supports publicly, which
 * is how the other twelve servers already test the same factory.
 *
...
export async function connectTestClient(credentials?: BConnectCredentials): Promise<Client> {
  ...
  const client = new Client({ name: "jobs-test-client", version: "1.0.0" }, { capabilities: {} });
