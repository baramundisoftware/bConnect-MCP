/**
 * bconnect-jobs-mcp — unified bootstrap (finding OPT-32)
 *
...
describe("bconnect-jobs-mcp bootstrap", () => {
  ...
  it("exits 1 on missing credentials instead of throwing a stack trace", async () => {
    ...
    expect(lines[0]).toBe(
      "bconnect-jobs-mcp: Either BCONNECT_API_KEY or both BCONNECT_USERNAME and BCONNECT_PASSWORD are required"
    );
  });
