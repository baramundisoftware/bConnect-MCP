/**
 * bconnect-groups-mcp — finding A2 / INT-53 regression: ONE error prefix, not two.
 *
...
describe('bconnect-groups-mcp — error message carries one MCP prefix', () => {
  ...
  it('invalid arguments: -32602 is prefixed exactly once', async () => {
    const error = await callAndCatch('list_group_members', {
      groupKind: 'logical',
      groupId: 'not-a-guid',
    });
  ...
  it('the TOK-22 refusals carry one prefix too — they are the new common rejection', async () => {
    // The unsupported-pair and inapplicable-filter refusals are new with the
    // collapse and are now the messages a migrating client will hit most, so
    // they get the same assertion rather than inheriting it by assumption.
    for (const args of [
      { groupKind: 'dynamic', groupId: '11111111-2222-3333-4444-555555555555', memberType: 'android' },
      { groupKind: 'logical', groupId: '11111111-2222-3333-4444-555555555555', memberType: 'ios', Domain: 'LABCORP' },
      { groupKind: 'logical', groupId: '11111111-2222-3333-4444-555555555555', DispalyName: 'x' },
    ]) {
      const error = await callAndCatch('list_group_members', args);
      expect(error.code).toBe(ErrorCode.InvalidParams);
      expect(error.message.match(/MCP error /g) ?? []).toHaveLength(1);
    }
  });
