...
describe('bconnect-assets-mcp', () => {
  ...
  describe('validator rejects bad arguments before reaching bConnect', () => {
    it('get_asset: missing id', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({ name: 'get_asset', arguments: {} })
      ).rejects.toThrow(/id is required/i);
    });

    it('create_asset: missing assetTypeId', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({
          name: 'create_asset',
          arguments: { ownerId: 'd0000001-0001-0001-0001-000000000001', ownerType: 'WindowsEndpoint', name: 'foo' }
        })
      ).rejects.toThrow(/assetTypeId is required/i);
    });

    it('update_asset: operations not an array', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({
          name: 'update_asset',
          arguments: { id: 'd0000001-0001-0001-0001-000000000001', operations: 'oops' }
        })
      ).rejects.toThrow(/operations/i);
    });

    it('list_assets_by_logical_group: missing logicalGroupId', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({ name: 'list_assets_by_logical_group', arguments: {} })
      ).rejects.toThrow(/logicalGroupId is required/i);
    });

    it('create_asset_type_folder: missing name', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({ name: 'create_asset_type_folder', arguments: {} })
      ).rejects.toThrow(/name is required/i);
    });

    it('list_asset_types: ShowSummary not a boolean', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({ name: 'list_asset_types', arguments: { ShowSummary: 'yes' } })
      ).rejects.toThrow(/ShowSummary/i);
    });

    it('list_asset_stock_subfolders: includeSubfolders not a boolean', async () => {
      const { client } = await startServer();
      await expect(
        client.callTool({
