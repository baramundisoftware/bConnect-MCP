/**
 * bconnect-activedirectory-mcp — finding A2 / INT-53 regression: ONE error prefix, not two.
 *
...
describe('bconnect-activedirectory-mcp — error message carries one MCP prefix', () => {
  ...
  it('invalid arguments: -32602 is prefixed exactly once', async () => {
    const error = await callAndCatch('get_ad_user', { id: 'not-a-guid' });
