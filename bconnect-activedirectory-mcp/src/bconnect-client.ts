...
export class BConnectClient {
  ...
  public activeDirectory: ActiveDirectoryModule;
  ...
  constructor(config: BConnectConfig) {
    ...
    this.activeDirectory = new ActiveDirectoryModule(this.client);
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      await this.activeDirectory.getADGroups({ PageSize: 1 });
      return true;
    } catch (error) {
