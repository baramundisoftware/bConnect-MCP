...
export interface BConnectConfig {
  ...
  // V1.1 API Configuration
  v1BaseUrl?: string; // V1.1 API base URL (optional, defaults to /baramundi)
...
export class BConnectClient {
  private client: AxiosInstance; // V2.0 client
  private v1Client: AxiosInstance; // V1.1 client
  ...
  public defenseControl: DefenseControlModule;
  ...
  constructor(config: BConnectConfig) {
    ...
    // Only add httpsAgent if not explicitly disabled (needed for MSW testing)
    if (!config.disableHttpsAgent) {
      v1AxiosConfig.httpsAgent = new https.Agent(httpsAgentOptions);
    }

    this.v1Client = axios.create(v1AxiosConfig);

    // Setup Basic Authentication for both clients
    this.setupAuth();
    ...
    // Setup retry logic for V1.1 client (same strategy as V2.0)
    axiosRetry(this.v1Client, {
    ...
    if (config.rateLimit?.enabled) {
      ...
      // Add rate limiting request interceptor for V1.1 client
      this.v1Client.interceptors.request.use(
        (requestConfig: InternalAxiosRequestConfig) => {
          if (this.rateLimiter) {
            const rateLimitInfo = this.rateLimiter.tryConsume();
            if (!rateLimitInfo.allowed) {
    ...
    if (config.auditLog?.level && config.auditLog.level !== 'none') {
      ...
      // Add audit logging request interceptor for V1.1 client
      this.v1Client.interceptors.request.use(
        (requestConfig: InternalAxiosRequestConfig) => {
          if (this.auditLogger) {
            const method = requestConfig.method?.toUpperCase() || 'GET';
    ...
    if (config.cache?.enabled) {
      ...
      // Add cache request interceptor for V1.1 client
      this.v1Client.interceptors.request.use(
        (requestConfig: InternalAxiosRequestConfig) => {
          if (this.responseCache) {
            const method = requestConfig.method?.toUpperCase() || 'GET';
    ...
    // Setup error handling and rate limit headers interceptor for V1.1 client
    this.v1Client.interceptors.response.use(
      (response) => {
        // Check if response was cached (from request interceptor)
        if (this.responseCache && response.config) {
          const cachedResponse = (response.config as any).__cachedResponse;
    ...
    this.defenseControl = new DefenseControlModule(this.client);
  }
  ...
  private setupAuth() {
    ...
    this.v1Client.defaults.headers.common["Authorization"] = `Basic ${authString}`;
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      await this.defenseControl.getBitLockerWindowsEndpoints({ PageSize: 1 });
      return true;
    } catch (error) {
