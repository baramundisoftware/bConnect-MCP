...
**Tools:** 11 by default (14 with `ALLOW_WRITE_OPERATIONS=true`)
...
node --env-file=/path/to/bconnect.env bconnect-defensecontrol-mcp/build/index.js
```
...
```json
{
  "bconnect-defensecontrol": {
    ...
    "args": [
      ...
      "/opt/bconnect-mcp-suite/bconnect-defensecontrol-mcp/build/index.js"
    ]
  }
}
```
...
| `get_bitlocker_secrets` | Get BitLocker recovery keys and startup PIN |
| `update_bitlocker_pin` | Update the BitLocker startup PIN for an endpoint |
...
| `refresh_local_admin_account_expiry` | Force a baramundi client to refresh managed data |
...
| `get_security_posture` | Composite tool: combined BitLocker/LAPS/Defender security posture summary for an endpoint |
